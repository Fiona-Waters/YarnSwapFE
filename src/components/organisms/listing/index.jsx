import { Box, Card, CardBody, CardFooter, CardHeader, Image, List, ListItem, Text } from "@chakra-ui/react";

export function Listing(props) {

    const { listing } = props
    return (
        <Card maxW={72} minW={12} align={"center"}>
            <CardHeader>
                <Box>
                    <Image boxSize='170px' objectFit='cover' src="data:image/jpeg;base64," />
                    {listing?.image} </Box>
            </CardHeader>
            <CardBody>
                <List>
                    <ListItem>
                        <Text>Brand: {listing?.brand}</Text>
                    </ListItem>
                    <ListItem>
                        Colourway: {listing?.colorway}
                    </ListItem>
                    <ListItem>
                        Weight: {listing?.weight}
                    </ListItem>
                    <ListItem>
                        Fibre Content: {listing?.fibreContent}
                    </ListItem>
                    <ListItem>
                        Swappable:  Yes
                    </ListItem>
                    <ListItem>
                        Status: Available
                    </ListItem>
                </List>
            </CardBody>
            <CardFooter></CardFooter>
        </Card>
    )
}